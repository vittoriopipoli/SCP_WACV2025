...
class MTDataModule(LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if self.dist:
            self.train_sampler = DistributedSampler(self.train_dataset, shuffle=True, seed=self.config["seed"])
            self.val_sampler = DistributedSampler(self.val_dataset, shuffle=True, seed=self.config["seed"])
            self.test_sampler = DistributedSampler(self.test_dataset, shuffle=False, seed=self.config["seed"])
        else:
