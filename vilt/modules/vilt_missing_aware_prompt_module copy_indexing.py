...
# 
        ...
        # self.cal = ContextAwarePromptLayer2(config["hidden_size"], prompt_length*3, prompt_num)
        ...
        self.mergeprompts = torch.cat([  complete_prompt.unsqueeze(0),
                                    missing_text_prompt.unsqueeze(0),
                                    missing_img_prompt.unsqueeze(0)], dim=0)
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        #     prompt = prompt.unsqueeze(0).to(cuda_device)
        #     if idx == 0:
        #         prompts = prompt
        #     else:
        ...
        for idx in range(len(img)):
            # if batch["missing_type"][idx] == 0:
            #     prompt = self.mergeprompts[0].to(cuda_device)      
            # elif batch["missing_type"][idx] == 1:
            #     prompt = self.mergeprompts[1].to(cuda_device)   
            # elif batch["missing_type"][idx] == 2:
            #     prompt = self.mergeprompts[2].to(cuda_device) 
            index_type = int(batch["missing_type"][idx])      
            prompt = self.mergeprompts[index_type].to(cuda_device)    
