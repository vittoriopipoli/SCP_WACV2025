...
class ViLTransformerSS_QKV_ps_att(pl.LightningModule):
...
# 
        ...
        slack =0
        if self.prompt_layers[0]==0:
            self.add_agnostic_prompts = layers.add_agnostic_prompts(config["hidden_size"], prompt_length+self.fixed_prompts)
            slack=1
        ...
        for i in self.prompt_layers:
            if i!= 0:   
                layer = self.QKV_ps_layers[i-slack-self.prompt_layers[0]]  
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        if self.learnt_p:
            if self.prompt_type=='attention':
                ...
                prompt_masks = torch.ones(missing_type.shape[0], prompt_length*len(self.prompt_layers), dtype=missing_type.dtype, device=missing_type.device).long()
        else:
        ...
        for i, blk in enumerate(self.transformer.blocks):
            if i in self.prompt_layers:
                if self.multi_layer_prompt:
                    CLS_idx = (i-self.prompt_layers[0])*(self.prompt_length+self.fixed_prompts)
                    CLS = x[:,CLS_idx:CLS_idx+1] 
                    if self.prompt_layers[0]==0 and i==0:
                        selected_prompts = self.add_agnostic_prompts(x)
                        slack=1
                    else:
                        selected_prompts, A = self.QKV_ps_layers[i-slack-self.prompt_layers[0]](CLS, missing_type)
                    ...
                    CLS_idx = (i-self.prompt_layers[0]+1)*(self.prompt_length+self.fixed_prompts)
        ...
        if self.prompt_type == 'input':
            total_prompt_len = len(self.prompt_layers)* (self.prompt_length+self.fixed_prompts) #prompts.shape[-2]
