...
class ViLTransformerSS_embedding_mix(pl.LightningModule):
...
# 
        ...
        self.cal = ContextAwarePromptLayer(config["hidden_size"], prompt_length*3+self.fixed_prompts, prompt_num)
        ...
        agnostic         = torch.tensor([i for i in range(self.fixed_prompts)])+prompt_length*3
...
#                     image_embeds[idx,1:].fill_(1)
        ...
        for idx in range(len(img)):
            prompt = torch.cat([self.prompt_idx_dict[batch["missing_type"][idx]],self.prompt_idx_dict[3]], dim=0)
        ...
        if self.learnt_p:
            if self.prompt_type=='attention':
                prompt_masks = torch.ones(prompts.shape[0], (self.prompt_length+self.fixed_prompts)//2, dtype=prompts.dtype, device=prompts.device).long()
            elif self.prompt_type=='input':
                prompt_masks = torch.ones(prompts.shape[0], (self.prompt_length+self.fixed_prompts)*len(self.prompt_layers), dtype=prompts.dtype, device=prompts.device).long()
        else:
            prompt_masks = torch.ones(prompts.shape[0], (self.prompt_length+self.fixed_prompts), dtype=prompts.dtype, device=prompts.device).long()   
